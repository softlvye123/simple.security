...
public class UnsafeSecurityFactoryByBytesTest {
    // RSA
    ...
    public void setUp() throws Exception {
        ...
        privateKeyEncrypt = UnsafeSecurityFactory.getEncryptInstanceByBytes(
                EncryptType.RSA_PKCS1_PRIVATE, Constants.rsaPrivateKeyBytes);

        publicKeyDecrypt = UnsafeSecurityFactory.getDecryptInstanceByBytes(
                DecryptType.RSA_X509_PUBLIC, Constants.rsaPublicKeyBytes);

        publicKeyEncrypt = UnsafeSecurityFactory.getEncryptInstanceByBytes(
                EncryptType.RSA_X509_PUBLIC, Constants.rsaPublicKeyBytes);

        privateKeyDecrypt = UnsafeSecurityFactory.getDecryptInstanceByBytes(
                DecryptType.RSA_PKCS1_PRIVATE, Constants.rsaPrivateKeyBytes);

        privateKeySign = UnsafeSecurityFactory.getSignInstanceByBytes(
                SignType.RSA_PKCS1_PRIVATE, Constants.rsaPrivateKeyBytes);

        publicKeyVerify = UnsafeSecurityFactory.getVerifyInstanceByBytes(
                VerifyType.RSA_X509_PUBLIC, Constants.rsaPublicKeyBytes);

        aesEncrypt = UnsafeSecurityFactory.getEncryptInstanceByBytes(
                EncryptType.AES_PKCS5, Constants.secretyKeyBytes);

        aesDecrypt = UnsafeSecurityFactory.getDecryptInstanceByBytes(
                DecryptType.AES_PKCS5, Constants.secretyKeyBytes);

        desEncrypt = UnsafeSecurityFactory.getEncryptInstanceByBytes(
                EncryptType.DES, Constants.secretyKeyBytes);

        desDecrypt = UnsafeSecurityFactory.getDecryptInstanceByBytes(
                DecryptType.DES, Constants.secretyKeyBytes);

        md5Sign = UnsafeSecurityFactory.getSignInstanceByBytes(SignType.MD5,
                null);

        md5Verify = UnsafeSecurityFactory.getVerifyInstanceByBytes(
                VerifyType.MD5, null);
    }
